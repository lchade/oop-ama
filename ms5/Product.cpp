...
namespace ama {
    ...
    // returns the address of the string storing product name
    ...
    std::ostream& Product::write(std::ostream& out, int writeMode) const {
        if (!isClear()) {
            ...
            if (writeMode == write_table) {
                ...
                out << setw(6) << m_qty_needed << " |" ;
            }

            if (writeMode == write_human) {
                out << fixed << right << setw(max_length_label) << "Sku: "<< m_sku << endl;
                out << setw(max_length_label) << "Name: " << m_name << endl;
                out << setw(max_length_label) << "Price: " << setprecision(2) << m_price << endl;
                out << setw(max_length_label) << "Price after Tax: " << this->afterTax() << endl;
                out << setw(max_length_label) << "Quantity Available: " << m_qty_onhand << " " << m_unit << endl;
                out << setw(max_length_label) << "Quantity Needed: " << m_qty_needed << " " << m_unit << endl;
            }

            if (writeMode == write_condensed) {
                out << m_type << "," << m_sku << "," << m_name << "," << m_unit << "," ;
                out << fixed << setprecision(2) << m_price << "," << m_tax << "," << m_qty_onhand << "," << m_qty_needed;
            }
        } 
        return out;
    }
